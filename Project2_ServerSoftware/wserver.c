...
request_t get_sff_request()
{
  ...
  {
    // Otherwise, shift items to fill the gap
    for (int i = smallest_idx; i != buffer_tail;)
    {
      int next = (i + 1) % buffer_size;
      if (next == buffer_tail)
      {
    ...
    buffer_tail = (buffer_tail == 0) ? (buffer_size - 1) : (buffer_tail - 1);
  }
